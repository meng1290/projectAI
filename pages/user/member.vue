          ...
          >
						<view class="li_box">
							<view class="li_l">
								...
								<view
									v-if="item.originalPrice && item.originalPrice !== item.displayPrice"
...
<script setup>
	...
	import { memberRight, memberShip, appleVerify } from "@/api/index.js"
	...
	const getDataList = async () => {
		try {
			const res = await memberShip({type:isIOS.value?'ios':'android'})
			if(res && res.length){
				dataList.value = res.map(item => ({
					...item,
					displayTitle: item.title,
					// 默认使用后端价格（优先 price，避免 0 显示）
					displayPrice: `￥${item.price || item.pre_price}`,
					originalPrice: item.price ? `￥${item.price}` : '',
					hasDiscount: parseFloat(item.pre_price) < parseFloat(item.price)
				}))
			}
		} catch (error) {
	...
	const initIAPAndMergeProducts = async () => {
		try {
		// 1. 初始化IAP：仅使用后端返回的 ioskey
			const appleProductIds = Array.from(
				new Set(
					dataList.value
					.map(item => item.ioskey)
						.filter(Boolean)
				)
			)

			if (!appleProductIds || appleProductIds.length === 0) {
			throw new Error('未找到可用的苹果商品ID，请检查后端 ioskey 配置')
			}
			...
			dataList.value.forEach(backendItem => {
				const appleProductId = backendItem.ioskey
				...
				if (appleProductId && appleProductMap[appleProductId]) {
					...
					// 处理苹果价格，确保格式统一
					let formattedPrice = appleProduct.price || appleProduct.localizedPrice || appleProduct.priceLocale || ''
					...
					if (formattedPrice && !formattedPrice.includes('￥') && !formattedPrice.includes('¥')) {
						...
						if (priceMatch) {
							...
							formattedPrice = `￥${backendItem.price || backendItem.pre_price}`
						}
					...
					mergedList.push({
						...
						applePrice: formattedPrice,
						// 显示价格：统一使用苹果价格，确保包含￥符号
						displayPrice: formattedPrice,
						// 原价使用后端返回的 price（用于划线显示）
						originalPrice: backendItem.price ? `￥${backendItem.price}` : '',
						// 苹果商品ID
	...
	const handleApplePay = async (selectedPackage) => {
		...
		try {
			...
			// 支付成功后的操作
			// iOS 支付成功后由 payResult 控制跳转，不再额外返回上一页
	...
	const verifyApplePayment = async (paymentResult, selectedPackage) => {
		console.log('verifyApplePayment',paymentResult,selectedPackage)
		try {
			// 获取收据数据
			...
			// 	}
			// })
			const verifyRes = await appleVerify({receipt: receipt,product_id:selectedPackage.appleProductId})
